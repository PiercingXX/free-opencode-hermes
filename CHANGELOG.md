...
- **No paid-product brand ads.** Dropped two paid-product design systems,
  ...
  model ids because those are the API names.

- **OpenCode tool calls round-trip.** The Responses adapter no longer puts
  function-call arguments on both `output_item.added` and the argument delta
  (OpenCode concatenates those and aborts). Object arguments, consecutive
  `function_call` items, and XML/text tool dumps are converted onto Chat
  Completions without concatenating empty arguments onto the first delta. Empty `read` becomes `glob *` instead of Read `.`, which OpenCode also
  aborts. Install no longer copies `build.md` / `plan.md` / `general.md` over
  OpenCode's native agents. `foc-opencode` and the plugin unlink those leftover
  files on launch so an existing install does not keep serving xx-stack
  `build.md`.
- **Hermes `:8180` streams.** `stream: true` on openai-compatible lanes is
  forwarded upstream and the SSE body is relayed. A JSON completion still
  uses the single-chunk shim; `hermes_cli` lanes do too.
- **MCP tests isolate `$HOME`.** The suite no longer needs write access to the
  developer's `~/.config/opencode/`.
- **xx-stack agents on plain `opencode`.** The plugin injects the nested
  xx-stack agents again when you launch `opencode` (not only `foc-opencode`).
  Native `build` / `plan` / `general` still have no markdown prompt overlay.
- **xx-stack OpenCode host (from upstream 85ed85c).** Slash commands
  (`/review`, `/plan`, `/debug`, `/ship`, `/explore`, `/route`, `/judge`),
  Tab-able `research`, restored supervisor tools on `execution-orchestrator`,
  and a real shared-instructions contract. Lease write-backs cannot smuggle a
...
- **Catalog routing and loopback launchers.** OpenCode's picker is
  `free-opencode/default`; Admin chooses the real upstream and fallbacks.
  `foc-opencode` / `foc-hermes` write process-local config (Responses on
  `:8082`). Self-hosted Connect covers Ollama, SGLang, and Tailscale. A stale
  detached proxy is replaced when its `builtAt` is older than the current
  build.

- **Vendored xx-stack 1.65 + 1.66, kept nested.** Dropped the shipped VS Code
  surface (`adapters/`, `vscode/`, `setup-vscode.sh`). MCP tools folded into
  `route_task` / `check_health` / agent getters. Tests compile to `dist-test/`.
  Hermes orchestrator split into `hermes_lib/`. Prompt tools live in the
  `compose-supervisor-prompts` skill. Clone-only MCP package. This repo does
  **not** flatten `xx-stack/` to the git root. Inventory scripts and MCP
  reliability tests resolve git-root files (`inventory.json`,
  `opencode-orchestration/`, `hermes-orchestration/`) from the nested tree.
  `opencode-orchestration/hooks` is a symlink onto `xx-stack/hooks`.

- **Evidence lanes, generations, canaries, and QD-lite** (from the xx-stack
  1.65 snapshot). A finding store (`finding_record`, `finding_list`,
  `generation_open`, `generation_close`, `generation_status`) records results
  as confirmed / incubator / diagnostic. Force-synthesized salvage lands in
  incubator as `partial_output` and cannot be marked completed.
