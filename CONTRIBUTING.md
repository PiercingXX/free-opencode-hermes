...
2. Clone your fork: `git clone https://github.com/YOUR_USERNAME/free-opencode-hermes.git`
...
# Full verification gate — layout, drift, design, lint, and every test suite
npm run verify
```

`npm run verify` is the local gate. GitHub Actions in this repo is **disabled**
(`workflow_dispatch` only in `.github/workflows/ci.yml`); a green local `verify`
is what you run before a PR. The workflow file is still kept in parity with
`verify` so it can be turned back on: **if you add a gate to `verify`, add it
to `ci.yml` in the same commit.** `ci:parity` enforces that.

When Actions is enabled, it additionally:

- runs the MCP server suite on **Node 20 and 22**; `verify` runs it on
  whatever Node you have
- regenerates `DESIGN-CATALOG.md` (`npm run design:catalog`) and fails if the
  committed copy is stale (`verify` does not, because it mutates the tree)
- runs `npm audit --omit=dev --audit-level=high` (needs the network; see
...
The pre-commit hook runs `npm run drift:check` so OpenCode agent and skill
mirrors have not drifted from `xx-stack/runtime/`. The VS Code
`agents:check` / `agents:sync` surface was removed in xx-stack 1.65.
...
and are gated by `npm run drift:check`. The VS Code / Copilot adapter surface
was removed in xx-stack 1.65; OpenCode is the install layer.
...
`*.nano.md` variants are derived tiers, not agents, and are covered by
`npm run nano:check`.
...
4. Add a `xx-stack/packs/rules/coverage.json` entry
...
```bash
npm run verify        # full local gate (layout, drift, lint, design, all tests)
...
The MCP server suite isolates `$HOME` and `XDG_CONFIG_HOME` to a temp
directory (`xx-stack/mcp-server/test-isolate-home.mjs`) so it does not write
the developer's `~/.config/opencode/`.
...
3. Ensure `npm run verify` is green (GitHub Actions is workflow_dispatch-only)
