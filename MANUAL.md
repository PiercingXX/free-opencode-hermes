...
packages/plugin/           Free OpenCode plugin + loopback catalog proxy
```
...
`xx-stack/runtime/config.json`. OpenCode copies live in
`opencode-orchestration/opencode/agents/` and are gated by
`npm run drift:check` — never treat the copies as canonical. The VS Code
adapter surface (`adapters/`, `vscode/`, `setup-vscode.sh`,
`sync-vscode-agents.mjs`) was removed in xx-stack 1.65.

Inside OpenCode the shipped host contract is
`opencode-orchestration/opencode/config.json`: `default_agent` is `build`,
...
still not copied over the built-in agents.
...
That distinction matters for how you treat it. A byte-level audit found 136 of
149 `design-systems/` files identical to their source at the pinned commit
...
our design systems name `#000000` as deliberate brand vocabulary, and the
...
`stream: true` on openai-compatible lanes is forwarded upstream and the SSE
body is relayed. A JSON completion (and `hermes_cli` lanes) still use the
single-chunk SSE shim.
...
```
layout:verify → drift:check → rules:check → nano:check
              ...
              → design:anti-slop-test → test → hermes:test → plugin:test
```

`agents:check` (VS Code agent mirrors) was removed with the VS Code surface in
xx-stack 1.65. `drift:check` is the remaining OpenCode mirror gate.
GitHub Actions is **disabled** (`workflow_dispatch` only); `npm run verify` is
the gate you run locally. `.github/workflows/ci.yml` is still kept in parity
with `verify` so it can be re-enabled.
...
| `test` | MCP suite (`$HOME` isolated to a temp dir) | see §11 for the classes it historically missed |
| `hermes:test` | Python suite, against the *shipped* allowlist | — |
| `plugin:test` | Free OpenCode plugin (Responses proxy, launchers, Admin) | live-model tool-call behaviour |
...
do fail), `design:catalog` staleness (it mutates the tree, so the workflow
checks it separately when Actions is enabled), and the Node 20/22 matrix.
GitHub Actions is currently `workflow_dispatch` only — run `npm run verify`
locally.
...
`drift:check` covers OpenCode agent and skill mirrors (names and content). The
VS Code `agents:check` surface (`runtime→adapters`, `opencode→vscode`,
`sync-vscode-agents.mjs`) was removed in xx-stack 1.65; those directories are
gone. Historical note: `vscode/agents/` used to be hand-maintained with no
sync and no check; when generation was switched on it turned out 8 of 18
agents were covered and the bodies were 20–45% shorter than their source —
the same defect class as §11 CONTENT-2, live on a surface `setup-vscode.sh`
copied into users' prompt directories.
...
`.githooks/pre-commit` runs `npm run drift:check` and nothing else. Enable it
...
These are still judgment calls, not correctness bugs. The 2026-09-01 pass did
not add new open rows.
...
3. Coverage entry, then `npm run drift:check` and `npm run verify`.
...
- Never treat `opencode-orchestration/opencode/agents/*` as canonical.
...
now: that gate compares names *and* content, normalizing only the deliberate
deltas. If behavior still diverges, the remaining difference is one of those
deltas (a `model:` pin or an OpenCode permission rewrite) or something outside
the mirrored files.

**`drift:check` fails on an agent I just added.** Working as intended. Add the
OpenCode mirror under `opencode-orchestration/opencode/agents/` (and register
it there) in the same commit. The VS Code `agents:sync` surface no longer
exists.
