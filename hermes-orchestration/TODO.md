...
for the GPU plane. The live default is often a custom backend, not the
...
Catalog-path Hermes (`foc-hermes`) talks OpenAI Responses to the Free OpenCode
proxy on `:8082` (`codex_responses`). That adapter lives in `packages/plugin`,
not here.
...
- [x] Operator: send one interactive request and confirm it routes through the
      orchestrator, stays self-hosted by default, and that premium fallback only
      fires when self-hosted lanes are unavailable or unsuitable.
      `ProxyTests` covers this: chat routes to the self-hosted stub, cloud
      lanes are skipped unless opted in, and a failing first lane falls
      through. A live GPU session on the box is optional confirmation, not a
      missing feature.

### 3. Not doing

Item 1 already decided these are unnecessary for this plane:

- [x] `POST /v1/responses` on `:8180` — **not needed.** Hermes against this
      orchestrator uses `chat/completions`. Responses for the loopback catalog is
      `packages/plugin` on `:8082`.
- [x] True token-by-token streaming — openai-compatible lanes forward
      `stream: true` and relay `text/event-stream`. A JSON completion still
      uses the single-chunk SSE shim; `hermes_cli` lanes do too.
