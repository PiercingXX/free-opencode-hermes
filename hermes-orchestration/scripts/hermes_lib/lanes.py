...
def _http_error_body(error: urllib.error.HTTPError) -> Tuple[int, Dict[str, Any]]:
...
def http_json(
    ...
    req = _http_request(method, url, payload, api_key)
    try:
...
def http_open(
    ...
    """Open an HTTP request without reading a successful body.

    Success (status < 400): ``(status, response)`` — caller must close
    ``response``. Failure: ``(status, parsed_json)`` — nothing to close.
    """
    req = _http_request(method, url, payload, api_key)
    try:
