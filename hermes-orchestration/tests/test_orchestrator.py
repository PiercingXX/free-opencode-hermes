...
class StubLaneHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.server.post_status != 200:
            ...
            return
        if payload.get("stream") and not self.server.stream_as_json:
    ...
    def _send_sse(self, payload):
        ...
        frames = [
            ...
            },
            {
                ...
                "created": 0,
                "model": model,
                "choices": [{"index": 0, "delta": {"content": self.server.reply}, "finish_reason": None}],
            },
            {
                "id": "chatcmpl-stub",
                "object": "chat.completion.chunk",
...
class ProxyTests(TempDirMixin):
    ...
    def _stream_chat(self, payload):
        ...
        return resp.status, resp.getheader("Content-Type") or "", resp.read().decode("utf-8")

    def test_chat_stream_relays_upstream_sse(self):
        status, content_type, raw = self._stream_chat(
            {"model": "auto", "stream": True, "messages": [{"role": "user", "content": "hi"}]}
        )
        self.assertEqual(status, 200)
        self.assertIn("text/event-stream", content_type)
        self.assertIn("proxied-reply", raw)
        self.assertIn("data: [DONE]", raw)
        self.assertGreaterEqual(raw.count("data: "), 3)
        self.assertEqual(self.stub.last_payload.get("stream"), True)

    def test_chat_stream_shims_when_upstream_returns_json(self):
        self.stub.stream_as_json = True
        status, content_type, raw = self._stream_chat(
            {"model": "auto", "stream": True, "messages": [{"role": "user", "content": "hi"}]}
        )
        self.assertEqual(status, 200)
    ...
    def test_chat_stream_falls_back_when_first_lane_fails(self):
        down = StubLaneServer(["model-down"], reply="nope")
        down.post_status = 500
        self.addCleanup(down.stop)
        self.cfg["lanes"]["down"] = lane_cfg(
            "lane-down", down.base_url, "model-down", priority=200
        )
        self.cfg["policy"]["primary_lane_order"] = ["down", "a"]
        status, content_type, raw = self._stream_chat(
            {"model": "auto", "stream": True, "messages": [{"role": "user", "content": "hi"}]}
        )
        self.assertEqual(status, 200)
        ...
        self.assertEqual(self.stub.post_count, 1)

    def test_chat_skips_cloud_lane_when_not_opted_in(self):
        cloud = StubLaneServer(["gpt-cloud"], reply="from-cloud")
        ...
        self.assertEqual(code, 200)
        self.assertEqual(body["choices"][0]["message"]["content"], "proxied-reply")
        self.assertEqual(cloud.post_count, 0)
        self.assertEqual(self.stub.post_count, 1)
        attempts = [
            attempt
            for record in self._routing_records()
            for attempt in record.get("attempts", [])
        ]
        self.assertTrue(any("cloud_not_allowed" in attempt for attempt in attempts))
