...
register_mcp_into_user_config() {
  # Nested layout: helper lives on the scripts symlink into xx-stack/, not
  # ../scripts (which is this repo's host-setup tree).
...
if [[ $GLOBAL -eq 1 ]]; then
  ...
  # Do not clobber OpenCode's native build/plan/general — those names replace
  # the built-in agent and empty Read/$ tool calls get aborted.
  rm -f "$USER_OPENCODE_DIR/agents/build.md" \
    "$USER_OPENCODE_DIR/agents/plan.md" \
    "$USER_OPENCODE_DIR/agents/general.md"
  for agent_file in "$OPENCODE_DIR"/agents/*.md; do
    [ -f "$agent_file" ] || continue
    base="$(basename "$agent_file")"
    case "$base" in
      build.md|plan.md|general.md) continue ;;
    esac
    cp -f "$agent_file" "$USER_OPENCODE_DIR/agents"/
  done
