...
function nextLogDir(): string {
  return resolve(homedir(), ".config/xx-stack/logs");
}

function legacyLogDir(): string {
  return resolve(homedir(), ".config/opencode/xx-stack-logs");
}

export function resolveLogDir(): string {
  const next = nextLogDir();
...
/** Snapshot for callers that import a path. Prefer resolveLogDir() if HOME can change. */
export const LOG_DIR = resolveLogDir();

function sessionsDir(): string {
  return join(resolveLogDir(), "sessions");
}

function serverLogPath(): string {
  return join(resolveLogDir(), "mcp-server.jsonl");
}
...
async function ensureLogDir(): Promise<void> {
  ...
  await __logIo.mkdir(sessionsDir(), { recursive: true });
  dirEnsured = true;
}
...
export function resolveSessionLogPath(sessionId: string): string | null {
  ...
  const candidate = resolve(dir, `${safe}.jsonl`);
  if (!candidate.startsWith(dir + sep)) return null;
  if (candidate.slice(dir.length + 1).includes(sep)) return null;
  return candidate;
}
...
export async function logEvent(
  ...
  try {
    ...
    if (stream === "server") {
      ...
      await rotateLargeLog(serverLog);
      const heal = (await needsLeadingNewline(serverLog)) ? "\n" : "";
      // The healing newline rides in the SAME append call. A separate append
      // re-opens the exact tear window it is closing.
      await __logIo.appendFile(serverLog, heal + line, "utf-8");
    } else {
...
export async function initServerLog(): Promise<LogEventResult> {
  try {
    await ensureLogDir();
    await rotateLargeLog(serverLogPath());
  } catch (error) {
