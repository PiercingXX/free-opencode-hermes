...
// Compiled tests live in mcp-server/dist-test/, so two levels up is the
// xx-stack component root. Free OpenCode nests that component under the git
// root; standalone xx-stack flattens it. inventory.example.json always sits
// at the git root and is the discriminator.
const xxStackRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..");
const gitRoot = existsSync(join(xxStackRoot, "inventory.example.json"))
  ? xxStackRoot
  : resolve(xxStackRoot, "..");
const runtimePlatformsPath = join(xxStackRoot, "runtime", "platforms.json");
const opencodePlatformsPath = join(gitRoot, "opencode-orchestration", "opencode", "platforms.json");
const hermesOrchestrationPath = join(
  gitRoot,
  "hermes-orchestration",
  "config",
  "orchestration.json"
);
const inventoryPath = join(gitRoot, "inventory.json");
const inventoryExamplePath = join(gitRoot, "inventory.example.json");
...
test("every shipped registry uses a valid endpointFamily", async () => {
  const registries = [
    { rel: "runtime/platforms.json", abs: runtimePlatformsPath },
    { rel: "opencode-orchestration/opencode/platforms.json", abs: opencodePlatformsPath },
  ];
  const valid = new Set(["ollama", "openai-compatible"]);

  for (const { rel, abs } of registries) {
...
async function loadJson(absPath: string): Promise<Record<string, any>> {
...
async function loadInventorySource(): Promise<{ raw: string; isTemplate: boolean }> {
  try {
    return {
      raw: await readFile(inventoryPath, "utf-8"),
      isTemplate: false,
    };
  } catch (err) {
    if ((err as NodeJS.ErrnoException).code !== "ENOENT") throw err;
    return {
      raw: await readFile(inventoryExamplePath, "utf-8"),
      isTemplate: true,
    };
  }
}
...
test("a machine's hardware is written once and inherited by all its runtimes", async () => {
  ...
  const registry = await loadJson(source.isTemplate ? runtimePlatformsPath : opencodePlatformsPath);
...
test("hermes lanes and TS hosts agree on endpoints for the same runtime", async () => {
  ...
  const hermes = await loadJson(hermesOrchestrationPath);
  const registry = await loadJson(source.isTemplate ? runtimePlatformsPath : opencodePlatformsPath);
...
test("the shipped example registry carries no personal hardware", async () => {
  const shipped = JSON.stringify(await loadJson(runtimePlatformsPath));
  const { raw, isTemplate } = await loadInventorySource();
  const inventory = JSON.parse(raw);
  const exampleIds = new Set(
    JSON.parse(await readFile(inventoryExamplePath, "utf-8")).machines.map(
      (m: { id: string }) => m.id
    )
  );

  // The shipped registry is generated from the template (see
  // generate-registries.mjs), so template machine ids are supposed to appear
  // there. Fail only when a personal inventory.json machine that is not in
  // the template leaked into the host-agnostic copy. A clone that still has
  // the example file as inventory.json is not a leak.
  if (isTemplate) return;

  for (const machine of inventory.machines) {
    if (machine.network.scope === "localhost" || machine.network.scope === "loopback") continue;
    if (exampleIds.has(machine.id)) continue;
    assert.ok(
...
test("cloud stays opt-out in both generated consumers", async () => {
  const registry = await loadJson(opencodePlatformsPath);
  const hermes = await loadJson(hermesOrchestrationPath);
...
test("the server starts when launched through a symlinked path", async () => {
  ...
  const symlinked = join(gitRoot, "opencode-orchestration", "mcp-server");
