...
all-pairs cap are a widely used dataviz-skill method, read at a published
bundle version `2.1.232`. That skill's reference palette **is not
reproduced**. Its values were run through its own validator while the craft
file was written and they do pass — worst adjacent CVD ΔE 9.1 light / 8.4
dark, worst adjacent normal-vision ΔE 19.6 light / 19.3 dark — but a
bundled skill ships no upstream `LICENSE` this repository can read, and this
...
- **`design-systems/`** — re-vendored 2026-08-03 at a **pinned** commit,
  `nexu-io/open-design` `e1c277c5` (subtree `049e1fa9b`). 136 of the 148
  open-design files remaining in the tree are byte-identical to that pin. Only **12** differ, and
  ...
  section upstream added after our snapshot. `bmw-m` is
...
by X" framing. That is wrong. The framing is upstream's, and 136 of 149 files
...
on day one and would otherwise be proving nothing.
